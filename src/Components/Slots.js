...
const handleShow = (num) => {
    ...
    //console.log(num);
    if(min < num ){
      alert(`Your booking slot  ${min} is available.  Please selct numeric order only.`)
    }else{
      setShow(true);
...
const onSubmitHandler = (event) => {
    ...
    alert(`The Person : ${name} Successfully alloted slot ${selectslotnum} `)
...
return (
    <Container>
    <Row>
        <h3 style={{textAlign:"center",marginTop:"20px",marginBottom:"20px",textDecoration:"underline"}}>Parking Slot Menu </h3>
        {
            arr.map((slot, index)=>{
                return(
